...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n", plays.get(p.getPlayID()).getName(), frmt.format(getAmount(p) / PERCENT_FACTOR), p.getAudience()));
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                ...
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    thisAmount += TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON * (performance.getAudience() - BASE_VOLUME_CREDIT_THRESHOLD);
                }
                break;
            case "comedy":
